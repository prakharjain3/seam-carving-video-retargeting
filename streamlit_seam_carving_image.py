...
from argparse_seam_carving_image import remove_horizontal, remove_vertical, add_horizontal, add_vertical
...
if uploaded_file is not None:
    ...
    desired_height = st.number_input("Desired Height", min_value=1, max_value=img_array.shape[0]*2, value=img_array.shape[0])
    desired_width = st.number_input("Desired Width", min_value=1, max_value=img_array.shape[1]*2, value=img_array.shape[1])

    logging.info(f"Original Height: {img_array.shape[0]} | Original Width: {img_array.shape[1]}")
    ...
    if st.button("Resize Image"):
        ...
        output_file = f"{basename}-result{ext}"
